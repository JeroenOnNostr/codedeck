...
- [x] **CD-057: start a new GSD project from the phone** — the goal the whole GSD strip effort was for, and the thing that was still impossible. Every session was rooted at `workspaceFolders[0]` (bridge **CDB-033**), so on a multi-project workspace GSD had nothing real to look at and `Start GSD` could only offer to initialise all 27 sibling repos as one project. New Session now takes a **Project folder** (blank = workspace root, unchanged) plus a **"Create it if it doesn't exist (new git repo)"** checkbox, sending `cwd` / `createCwd`. The bridge creates and `git init`s it — the `git init` is load-bearing, because `Start GSD` is gated on `hasGit` and a bare directory would arrive with the button dead. Field is hidden below protocol v8 rather than shown-and-ignored. Threaded through the optimistic-session record so a retry replays the same folder. (commits `916eee2` phone / `30a0338` bridge, 2026-07-28)

## Zapstore release 2026.07.29 — GSD start button + live/waiting/recovery states (2026-07-28)

- [x] **Published CodeDeck `2026.7.29` to Zapstore** — ships **CD-053** (GSD start button, per-session opt-in; waiting / live-execution / recovery states; pre-flight cost) on top of **CDB-032** (protocol v7). versionCode `20260729`, cert `c5d0cba4…fbc5`. **Version is one day ahead of the wall clock on purpose:** `2026.7.28` was already published earlier the same day, and calendar versioning has no intra-day slot — a suffixed code like `2026072801` would sort *above* every future `YYYYMMDD` and break monotonicity permanently, while republishing `2026.7.28` would leave existing installs with no update to see. Signer proven before publishing (`zsp publish -q --offline` → sole pubkey `f07e0b1a…c367`, the listing owner). **First publish attempt failed with a CDN `502` and published nothing** — the retry succeeded; never trust `-q` output, it mutes success lines and only echoes failures. Events landed `2026-07-28T15:36:24Z`; CDN blob `732f01bc…22fe` fetched back byte-identical, and the kind-3063 file event carries the matching `x`/`version_code`/cert. `zapstore.yaml` temp-injection reverted. Companion bridge bumped to `2026.7.29`; **the VSIX is still unpublished, so the GSD strip stays inert for users until it ships**. (2026-07-28)
